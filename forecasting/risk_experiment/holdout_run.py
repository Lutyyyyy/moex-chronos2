...
# (re-selected after the plan-H universe correction; U1 and U3 C replaced by F2 arms under the F rule, plan I)
FROZEN = {"U1": {"C": "mixeq_chr_F2ret", "B": "fhs_loghar", "refs": ("rm", "garch_t")},
          "U2": {"C": "fac_chr", "B": "port_loghar", "refs": ("ewma", "garch")},
          "U3": {"C": "mixeq_chr_F2rv", "B": "ewma", "refs": ("ewma", "garch")},
...
SOURCES = ("Z3", "N5m", "N5e")                     # zero-shot holdout sources the frozen arms need (run locally)
F_SOURCES = ("F2",)                                # fine-tuned sources the frozen arms need (Colab holdout bundle)
FT_HOLDOUT = rr.RUNS / "risk_ft" / "holdout"
F_HOLDOUT_YEARS = (2025, 2026)
...
def check_f_holdout(name: str, ft: pd.DataFrame, eligible: pd.DataFrame) -> None:
    """Coverage of the Colab holdout forecasts, checked before any scoring: every eligible (anchor, ticker) of
    the holdout, each variate of the target and h = 1..H, exactly once (the dev folds had exactly this, since
    they matched their zero-shot twin generated by the same function)."""
    import ft_core as fc
    keys = ["anchor", "ticker", "variate", "h"]
    anchors = rr.span(eligible.index, rr.HOLDOUT)
    el = eligible.loc[anchors].stack()
    cells = el[el].index
    want = pd.MultiIndex.from_tuples([(a, t, v, h) for a, t in cells for v in fc.TARGETS[name] for h in range(1, fc.H + 1)],
                                     names=keys)
    got = pd.MultiIndex.from_frame(ft[keys].assign(anchor=pd.to_datetime(ft["anchor"])))
    miss, extra = want.difference(got), got.difference(want)
    if got.has_duplicates or len(miss) or len(extra):
        raise ValueError(f"{name} holdout: missing {len(miss)}, extra {len(extra)}, duplicated {int(got.duplicated().sum())} rows")


def load_sources(period: str, eligible: pd.DataFrame | None = None) -> dict:
    """Dev forecasts; for the holdout, dev + holdout forecasts concatenated (the panels agree on the overlap,
    holdout_qa), so rolling calibrations at the start of 2025 see late-2024 forecasts. Fine-tuned sources are
    the spliced dev source (plan I) followed by the Colab holdout folds."""
    dev = {"Z3": rr.SRC["Z3"], "N5m": rr.source_path("N5m"), "N5e": rr.source_path("N5e")}
    P = {k: pd.read_parquet(v) for k, v in dev.items()}
    P.update({k: rr.load_f_source(k) for k in F_SOURCES})
    if period == "holdout":
        h0 = pd.Timestamp(rr.HOLDOUT[0])
        for k in SOURCES:
            h = pd.read_parquet(holdout_source_path(k))
            P[k] = pd.concat([P[k][P[k]["anchor"] < h0], h], ignore_index=True)
        for k in F_SOURCES:
            rr.ft_records(FT_HOLDOUT, (k,), years=F_HOLDOUT_YEARS)            # folds present, lr = validation argmin
            ft = pd.concat([pd.read_parquet(FT_HOLDOUT / k / str(y) / "preds.parquet") for y in F_HOLDOUT_YEARS], ignore_index=True)
            ft["anchor"] = pd.to_datetime(ft["anchor"]).astype(P[k]["anchor"].dtype)
            check_f_holdout(k, ft, eligible)
            P[k] = pd.concat([P[k][P[k]["anchor"] < h0], ft[P[k].columns]], ignore_index=True)
    return P
...
def build_arms(period: str, D, inb) -> dict:
    ...
    P = load_sources(period, D["eligible"])
    A = {}
    # U1 (as build_u1_arms)
    ...
    A["fhs_loghar"] = (V, E)
    Vc, Ec = ra.chronos_vares(P["F2"], (0.05,), cal, cols, variate="ret", h=1)[0.05]           # as build_f_arms
    A["mixeq_chr_F2ret"] = (rl.vincentize([Vc, V], [0.5, 0.5]), rl.vincentize([Ec, E], [0.5, 0.5]))
    ...
    A["mixeq_chr_F2rv"] = rl.geo_mix([ra.chronos_rv(P["F2"], steps, cal, cols, variate="logrv"), Vc5["loghar_pooled_mkt"]], [0.5, 0.5])
...
def evaluate(period: str) -> dict:
    ...
    n1 = (FROZEN["U1"]["C"], FROZEN["U1"]["B"], *FROZEN["U1"]["refs"])
    for k in n1:
        V, E = A[k]
        M &= V.lt(0) & E.lt(V)
    L = {k: rl.fz0_panel(y1, *A[k], M, 0.05) for k in n1}
    ...
    U = D["eligible"] & ine & R5.notna() & A["ewma"].gt(0) & A["garch"].gt(0) & A[FROZEN["U3"]["C"]].gt(0) & A["fac_chr"][1].notna()
    ...
    drd = ra.portfolio_paths({k: A[k] for k in ("ewma", "garch", FROZEN["U3"]["C"])}, corr, R5, rf5, U, dates, target5)
    ...
    res["U3"] = claims("U3", {k: paths[k]["gmv"] ** 2 for k in (FROZEN["U3"]["C"], "ewma", "garch")}, delta.get("U3"), S)
