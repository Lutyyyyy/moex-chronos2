...
- **`config.md` expanded to the full 80-ticker universe for Phase C (2026-09-17)**: `tickers.shares` widened from the 22-ticker Phase B subsample to all 80 tickers in `equity_universe.yaml`, for Phase C's lead-lag cross-correlation screen (needs the widest reasonable universe, up to 3160 pairs — proceeding despite Phase B's gate failing, per the user's explicit reasoning that Phase C tests a different question than Phase B did; see `../moex-hack`'s local pivot plan rev. 3). `datasets`/`candles.intervals` unchanged. X5/RAGR (known zero-history) still present, untrimmed.
