...
- **Equity universe selection (2026-09-17, for `../moex-hack`'s Path A pivot)**: `rank_equity_universe()` + `save_equity_universe()` added to `src/algopack_pipeline.py` (after `universe_report()`/`universe_markdown()`). Filters TQBR shares to real equities via `INSTRID == "EQIN"` (ETFs/funds carry `INSTRID == "IFTF"` — confirmed live against `securities.json`, which was not previously in the pulled column set; `INSTRID` added to `universe_report()`'s shares columns). Ranks survivors by `VALTODAY` after a daily-candle history/missingness check. Full audit trail (every candidate + status), not just survivors — see `docs/usage.md` §6. Run 2026-09-17: 506 candidates → 80 selected, output at `data/universe/equity_universe.yaml`.
- **`config.md` scoped for Phase B gate (2026-09-17)**: `tickers.shares` expanded from the original 10-ticker panel to a 24-ticker stratified subsample of `equity_universe.yaml`'s 80 (top-liquidity pick per sector across 12 hand-classified sectors — sector map is manual/best-effort, not from MOEX metadata). `datasets`/`candles.intervals` scoped down to `[candles]`/`[1d]` (from all 7 datasets / 4 intervals) to match Phase B/C's actual near-term needs; widen later if a phase needs richer covariates. Old config kept as documented history in `config.md`'s "Panel rationale" section. 47/47 offline tests still pass (config-only change). **Phase B ran and its gate FAILED** (chance-level DA both `group_mode` arms — see `../moex-hack` `docs/current_state.md` session entry 17).
- **`config.md` expanded to the full 80-ticker universe for Phase C (2026-09-17)**: `tickers.shares` widened from the 22-ticker Phase B subsample to all 80 tickers in `equity_universe.yaml`, for Phase C's lead-lag cross-correlation screen (needs the widest reasonable universe, up to 3160 pairs — proceeding despite Phase B's gate failing, per the user's explicit reasoning that Phase C tests a different question than Phase B did; see `../moex-hack`'s local pivot plan rev. 3). `datasets`/`candles.intervals` unchanged. X5/RAGR (known zero-history) still present, untrimmed. **Phase C ran to completion and its gate also FAILED** (20 discovery-shortlisted pairs, 0/20 replicated out-of-sample — see `../moex-hack` `path_a/scratchpads/phase_c_scratch_pad.md`).
- **`config.md`'s `candles.intervals` widened to `[1d, 1h]` for a post-Phase-C 1h follow-on (2026-09-17)**: adds 1h alongside the existing 1d (not replacing it). `period` deliberately left unchanged at the full 2020-2024 range rather than narrowed to the shorter window the 1h analysis will use — `period` applies globally across every interval, so narrowing it would silently truncate the already-processed, already-cited `candles_1d/shares.parquet` that Phase B/C's committed results are built on. Tickers unchanged (still the 80-ticker universe). 47/47 offline tests still pass (config-only change). **Phase C's 1h follow-on and Phase E (burst detection, daily+1h) both ran to completion and both FAILED** (see `../moex-hack` `docs/current_state.md` session entries 23/25-28) — five independent negative results total at daily/1h resolution, plus a per-window clustering check ruling out hidden non-pairwise structure in Phase B's multivariate arm.
- **`config.md`'s `candles.intervals` widened to `[1d, 1h, 10m]` for a 10-minute follow-on (2026-09-17)**: adds 10m alongside the existing 1d/1h (not replacing either). Same 80-ticker universe, same full 2020-2024 `period` (unchanged, same truncation-avoidance rationale as the 1h widening). Real added cost flagged before pulling: 10m needs ~3 ISS pages/month per ticker (vs 1h's ~1 page), so this interval alone costs roughly 3x the 1h pull's request volume (~2.5hr/~29k requests estimated, not yet run). 47/47 offline tests still pass (config-only change).
- **Dividend/split price adjustment added (2026-09-17)**: discovered via a data-quality check that no prior result (Stage 2b through Phase E) ever adjusted for dividends/splits — confirmed real via direct scan (MTSS dividend drops, BELU's 8-for-1 split). MOEX ISS has no free dividend endpoint (verified via research; only route is subscriber-only, neither `apimoex` nor `moexalgo` implement it). New `fetch_dividends()` in `src/algopack_pipeline.py` downloads/caches `poptimizer`'s community dividend dump (`https://raw.githubusercontent.com/WLM1ke/poptimizer/master/dump/dividends.json`, cross-checked against independently-verified MTSS/GAZP facts) to `data/dividends.json` (gitignored, regenerable — same convention as `equity_universe.yaml`). `compute_close_adj()` applies a standard backward-multiplicative total-return adjustment (dividends + a small manually-verified `KNOWN_SPLITS` table, currently just BELU) and is wired into `finalize()`/`build_processed()` for `candles/shares`, adding a new **additive** `close_adj` column — `close` and all other columns are untouched, so every prior committed result is unaffected. Adjustment logic lives here in `algo_data` (user's explicit choice), not in `../moex-hack/path_a` — Path A receives already-adjusted data. Applied going forward only, no retroactive re-run of prior results. New `tests/test_dividends.py` (5 tests); `test_pipeline_offline.py` extended with an injectable fake dividends fetcher so the offline suite stays fully offline. 50/50 offline tests passing. `tools/build_notebook.py` re-run.
- **Real-data verification of the adjustment (2026-09-17, same day)**: fetched the real poptimizer dump via `fetch_dividends()` (not a scratch file) and spot-checked BELU/MTSS against real raw daily chunks — **found and fixed a real bug**: `KNOWN_SPLITS`'s BELU date was wrong (2024-05-24, no price discontinuity there); the real split is 2024-08-22 (4680→714, verified). Corrected the code and its test, full suite re-verified. A broader 80-ticker market-relative excess-return scan post-adjustment flagged 159 residual (ticker, date) moves across 43 tickers — inspected the top of the list and found it dominated by known genuine market-wide events (2022-02-24 invasion, 2022-03-28/29/31 post-halt reopening); user's explicit decision: stop here, treat the residual list as informational, don't investigate further. `data/dividends.json` is now a real cache, but **the processed parquet files on disk still lack `close_adj`** — needs a real pipeline run to rebuild, not yet done (user runs pipeline executions, not this session).
