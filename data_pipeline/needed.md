...
- Panel: 10 shares + IMOEX + CNYRUB_TOM + continuous Si/BR/RI/GD (see `config.md`, `docs/universe.md`).
  **Superseded 2026-09-17** for `../moex-hack`'s Path A pivot: an 80-ticker equity universe is
  now available (`data/universe/equity_universe.yaml`, via `rank_equity_universe()`).
  `config.md`'s `tickers.shares` has since been updated to use it (see `config.md`'s own
  "Panel rationale" section for the history).
...
- Drive: `MyDrive/data_pipeline/` holds `config.md`, `.env` and `data/`.
